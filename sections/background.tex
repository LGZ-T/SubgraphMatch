...
\cparagraph{Partial embedding.} Given a query graph $q$ and a matching order $\pi$, \SystemName iteratively chooses  one or multiple
...
at the same time, but the techniques can be extended to more than two vertices (see \FIXME{Section \ref{}}).
...
\cparagraph{Handling graph symmetry.} If a query graph is symmetric \FIXME{\cite{}}, multiple embeddings can be isomorphic to the same
