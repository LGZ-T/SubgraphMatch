...
\FIXME{Talk about your overall performance - speedup before discussing things like the storage format.}
...
\begin{table}
\centering
  \caption{Space overehead of CSR, hash-PCSR (GSI), and our interval-PCSR. The \emph{Max} column represents the size of maximum edge label partition in
  the interval-PCSR. The \emph{spave reduction} column represents the percentage of the space saved given by our interval-PCSR over GSI's hash-PCSR.}
  \label{tab:graphsize}
  \begin{tabular}{llp{30pt}p{30pt}lr{30pt}}
  \toprule
    \textbf{Graph} &\textbf{CSR}&\textbf{interval-PCSR}&\textbf{hash-PCSR}&\emph{\textbf{Max}}&\emph{space reduction}\\
    \bottomrole
